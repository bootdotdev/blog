...
Several years ago I started my first job as a "senior" Go developer. Let's call the company, I don't know, "Parts Unlimited". After a meager three years in the industry, I had so blown Parts Unlimited's management team away with my arcane knowledge of the Go standard library's `strings.Contains()` function that I was fast-tracked to a life of luxury and ease.

Well, it *would* have been easy. It would have been easy if the mind-numbing, gelatinous, corporate goo known as *Java* (it runs on billions of devices btw) hadn't chipped away at the once great minds of my new colleagues.

We're going to talk about my struggles with unit tests at this company. And mocks. And heroine injections. Erm, I mean dependency injections. *(but tbh both are just as bad for you)*

**BUT...** before we do that, let me set the stage by providing some context about my work environment.
...
Needless to say, I was a gopher surrounded by a cacophony of eclipse IDEs. Before moving on though, I know you're dying to know... here's a rough implementation of Bill's "fix":
...
After a riveting discussion with Bill about the virtues of "errors as values", I was able to dive into my first (sigh) *Jira* ticket. I ran a few commands to get to work on an existing backend Go service:
...
*Why is my test suite trying to connect to a database server???*
...
At this point, you have the backstory, so it's time to introduce you to my philosophy on unit tests. Well, at least inasmuch as unit tests relate to databases.
...
Here's where I *really* disagree with Bill: I would actually *prefer* tests that run while connected to a real database. Though I'd *more* prefer *integration* tests that run separate from the unit tests... At least with a real database I'm testing code that actually *runs in production*.
...
Should be tested in an *integration* test. (or, * gasp * *manually*, you know, whatever). Howver, this *validation* logic can easily be tested in a unit test. You know, this part:
...
And because your code is ass, you shouldn't be scared to make some changes! Sure, this function will "require" a database connection in order to be tested:
...
```
But what if we break it down into... mhhmmmm what should we call them... "units" maybe?
